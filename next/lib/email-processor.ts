...
export class EmailProcessor {
  ...
  private async findOrCreateCompanyAndContact(emailContact: { name: string; email: string }) {
    ...
    let company: CompanyWithContacts | null = await prisma.company.findFirst({
      where: {
        OR: [
          { domain: domain },
    ...
    if (!company) {
      ...
      company = createdCompany as CompanyWithContacts;
    } else {
  ...
  private mapImportance(importance: string): $Enums.Importance {
    switch (importance.toLowerCase()) {
      case 'high':
        return 'HIGH' as $Enums.Importance;
      case 'low':
        return 'LOW' as $Enums.Importance;
      default:
        return 'NORMAL' as $Enums.Importance;
    }
  }
}
